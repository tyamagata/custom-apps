...
class AggregateMultiplatformServerToServerSenderException(Exception):
    pass


class AggregateMultiplatformServerToServerSender:
    ...
    def send(self):
        ...
        for row in self.events:
            self.to_aggregate_multiplatform_s2s_event(row)
            response = session.post(self.API_URL, data=json.dumps({'events': [(row)]}), headers=headers)
            if response.ok:
                successes += 1
            else:
                failures += 1
    ...
    def to_aggregate_multiplatform_s2s_event(self, row):
        for field in self.REQUIRED_FIELDS:
            if row.get(field) is None:
                raise Exception(f'Required field: {field}')

        aggregate_multiplatform_s2s_event = {}
        for key, value in row.items():
            if key in self.OPTIONAL_FIELDS or key in self.REQUIRED_FIELDS:
                aggregate_multiplatform_s2s_event[key] = value

        return aggregate_multiplatform_s2s_event
    ...
    def _get_api_token(self):
        ...
        if var:
            ...
            raise AggregateMultiplatformServerToServerSenderException('Missing API token.')
