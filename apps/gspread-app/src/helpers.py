...
secrets_path_users = os.getenv("SECRETS_PATH_USERS")
secrets_path_worksheets = os.getenv("SECRETS_PATH_WORKSHEETS")
...
def get_users():
    with open(secrets_path_users, "r") as f:
        sheet_passwords = json.load(f)
    return sheet_passwords
...
def get_worksheets_config():
    with open(secrets_path_worksheets, "r") as f:
        worksheets_config = json.load(f)
    return worksheets_config
